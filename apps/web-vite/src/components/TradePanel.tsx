...
export function TradePanel({ market }: TradePanelProps) {
  ...
  const { isApproved, needsApproval, checkApproval } = useApprovalCheck({
    targetContract: import.meta.env.VITE_MARKET_FACTORY_ADDRESS!,
  });
    ...
    try {
      ...
      if (activeTab === "buy") {
        ...
        const factoryContract = getContract({
          client,
          chain: defineChain(parseInt(import.meta.env.VITE_CHAIN_ID || "84532")),
          address: import.meta.env.VITE_MARKET_FACTORY_ADDRESS! as `0x${string}`,
        });
